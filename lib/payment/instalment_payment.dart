...
class Installment_Payment extends StatefulWidget {

  late int index;


  Installment_Payment(this.index);

  @override
  _MyHomePageState createState() => _MyHomePageState(index);
}

class _MyHomePageState extends State<Installment_Payment> {
    ...
    ?Text("Error, select card first")
        :Text("Error, Value is 0"),
                      ...
                      if(res.status=="initiated"){
                        appointmentCOntroller.bookAppointmentByInstallment(
                            appointmentCOntroller.appointmentsList.value[index].appointmentId
                        ,amount);
                        print("yes");
                        print(res.id);
                        showDialog(
